...
**// DESCRIPTION: Executes the implementation plan from tasks.md verbatim, performing only the code edits and checks that the plan prescribes. Do not re-interpret requirements, re-read project docs, or explore unrelated code—treat tasks.md as the sole source of truth.**

**1.0 Invocation & Inputs**
   - `path-to-spec-agent-output`: `.codex-flow/runtime/speckit/spec-agent-output.md`, which must be updated with implementation progress and proof of executed checks.
...
4. Do not expand scope based on design.md, requirements.md, README, or any other repo doc; only open those references when a task explicitly tells you to cite a requirement ID. Otherwise, start coding immediately per tasks.md instructions.
...
1. For each task (including Cross-phase tasks), rely solely on the wording inside that task: gather only the cited requirement/design IDs (without re-reading the source docs), open the listed files, and implement the described changes exactly.
...
6. Update `.codex-flow/runtime/speckit/spec-agent-output.md` after each phase with: completed task IDs, commands run, their results, outstanding blockers, and whether additional approvals are needed.
...
- `.codex-flow/runtime/speckit/spec-agent-output.md` reflects the implementation status, commands executed, their outcomes, and remaining work.
