...
3. Input definitions:
   ...
   - `requirements.md`: structured requirement set at `specs/<spec-name>/requirements.md`, produced by the requirements agent via `templates/requirements.md`.
...
2. Use `templates/design.md` to structure the artifact; keep every section and heading in order, populating them with project-specific content.
...
9. Update `.codex-flow/runtime/speckit/spec-agent-output/<spec-name>.md` to record the new design status, summarize critical architecture choices, and link to `specs/<spec-name>/design.md` for downstream agents.
