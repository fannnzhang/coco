...
- 一 Spec 一分支：始终使用 `spec/<spec-name>` 分支，并将产出写入 `.codex-flow/runtime/specs/<spec-name>/`。
- 模板先行：`requirements.md`、`design.md`、`tasks.md` 只能由 `.codex-flow/prompts/speckit/templates/*.md` 实例化，章节顺序不可破坏。
...
1. `.codex-flow/input/specification.md` — demand packets (Markdown). Read every file recursively in lexical order.
2. Speckit context registry: `.codex-flow/runtime/speckit/spec-agent-output.md` files. Treat this folder as the lookup table that maps demand packages to existing specs; only open the entries that match the current spec.
3. Targeted repo context: `README.md`, `docs/**`, crate-level `README`s explicitly cited by the demand packet, existing `.codex-flow/runtime/specs/<spec-name>/` content once the spec is identified, and `codex-flow-design.md` for system-wide constraints. Do not crawl unrelated directories.
...
- `.codex-flow/runtime/speckit/spec-agent-output.md` — canonical kickoff brief produced by this agent (schema in Section 5). This registry entry is the source of truth for downstream agents and the directory lookup for the spec workspace.
- `.codex-flow/runtime/specs/<spec-name>/requirements.md` — generated by the Requirements agent, must follow the template and enumerate every discrete requirement with EARS-style acceptance criteria.
- `.codex-flow/runtime/specs/<spec-name>/design.md` — generated by the Design agent, provides the holistic technical approach mapped to all requirements.
- `.codex-flow/runtime/specs/<spec-name>/tasks.md` — generated by the Tasks agent, delivers the phased engineering checklist for implementation.
...
3. Look up `.codex-flow/runtime/speckit/spec-agent-output.md`. If present, treat it as the authoritative record of the branch, workspace path, and prior artifact status—use it to confirm you are targeting the correct spec without scanning other directories.
...
1. Desired branch = `spec/<spec-name>`.
    ...
    - Else create from `origin/feature/flow` (or `feature/flow` if remote missing): `git checkout -b spec/<spec-name> origin/feature/flow`.
...
1. Read all demand files plus the matching `.codex-flow/runtime/speckit/spec-agent-output.md` (if it exists) to reuse past context.
...
5. Write `.codex-flow/runtime/speckit/spec-agent-output.md` following Section 5. The brief now doubles as the kickoff report, so capture:
...
2. Ensure `.codex-flow/runtime/speckit/spec-agent-output.md` reflects the latest evidence table, status summaries, and next actions so downstream agents have an accurate kickoff state.
...
- File Path: .codex-flow/runtime/specs/<spec-name>/
...
- [ ] `.codex-flow/runtime/speckit/spec-agent-output.md` refreshed with the kickoff brief schema (timestamp, sources, artifact status, risks, next actions).
...
**8.0 Git & File Guardrails**
- Never force-push or rewrite history.
- Limit edits to `.codex-flow/runtime/specs/<spec-name>/` and `.codex-flow/runtime/speckit/spec-agent-output.md` unless additional directories are explicitly required.
