...
- `workflow.steps[*].description`：可选的人类可读描述，用于 CLI 输出与 `.codex-flow/memory/` 日志文件命名；留空时退回到 `agent` 名称。
...
- `codex-flow run <workflow.toml> [--mock | --no-mock] [--verbose]`
  ...
  - Mock（默认开启，除非 `--no-mock` 或配置中明确关闭）：不调用真实模型，仅打印“将要执行的完整 Shell 命令”。
...
- **执行日志落盘**：每个真实步骤会将收到的 codex `--json` 事件实时写入 `.codex-flow/memory/{index}-{description}.json`（按步骤顺序编号，并使用 `description`/agent 名称生成文件名），便于回放与调试。
...
- MockEngine：
  - 不进行任何网络调用。
  - 响应策略：
    1) 若 `.codex-flow/mocks/<workflow>/<step>.md` 存在则读取返回；
    2) 否则基于 `prompt` 与 `input` 做最简模板渲染（例如输出一个带标题与变量占位的伪响应）；
    3) 或返回固定占位文本，附带日志，便于快照测试。
- CodexEngine（占位）：
