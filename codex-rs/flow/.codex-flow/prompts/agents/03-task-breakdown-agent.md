...
1.  **Initial Execution:**
    *   **Command:** `node .codex-flow/scripts/extract_tasks.js`
...
2.  **Verification and Remediation Cycle:**
    ...
    *   **Failure Condition & Remediation Loop:** If the output is incomplete (e.g., has empty fields like `"description": ""`) or is otherwise incorrect, you MUST initiate the following corrective loop:
        ...
        2.  **Modify:** Adjust the regular expressions or parsing logic within the `.codex-flow/scripts/extract_tasks.js` file to fix the identified error. Your goal is to improve the script's resilience and accuracy.
        3.  **Re-run:** Execute the modified script again using `node .codex-flow/scripts/extract_tasks.js`.
